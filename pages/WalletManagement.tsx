...
const WalletManagement: React.FC<{ state: AppState }> = ({ state }) => {
  ...
  const handleAutoRefill = async () => {
    ...
    setTimeout(() => {
      ...
      if (res.success) {
        db.logNotification(currentUser.email, 'success', 'Liquidity Refilled', `System protocol provisioned Rs. 10M to your administrative node.`);
      }
    }, 1000);
  };
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          <h2 className="text-3xl font-black text-slate-800 tracking-tight flex items-center gap-3 italic">
            ...
            Fiscal Handshake Hub
          </h2>
          <p className="text-slate-500 font-medium">Manage operational wallet liquidity and provision bandwidth credits across the network.</p>
        </div>
        <div className="flex bg-white p-1.5 rounded-[1.5rem] border border-slate-200 shadow-sm overflow-x-auto w-full md:w-auto">
          {[
            { id: 'liquidity', label: 'My Liquidity', icon: Globe },
            { id: 'personnel', label: 'Team Load', icon: UserCircle },
            { id: 'subscribers', label: 'User Provision', icon: HandCoins },
            { id: 'requests', label: 'User Requests', icon: Activity, count: pendingTopups.length },
          ].map(tab => (
            <button 
              key={tab.id} 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
           <div className="lg:col-span-2 bg-slate-950 rounded-[3rem] p-12 text-white relative overflow-hidden shadow-2xl flex flex-col justify-between min-h-[400px]">
              <div className="relative z-10 space-y-4">
                 <div className="flex justify-between items-start">
                    <div className="space-y-1">
                       <p className="text-[13px] font-black text-blue-400 uppercase tracking-[0.4em] italic">Operational Liquidity Node</p>
                       <div className="flex items-center gap-2">
                          <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div>
                          <span className="text-[9px] font-black uppercase text-slate-500">Registry Synchronized</span>
                       </div>
                    </div>
              ...
              <div className="relative z-10 flex flex-col sm:flex-row gap-4">
                 ...
                 >
                    {isRefilling ? <RefreshCw className="animate-spin" size={20}/> : <Zap size={20} fill="currentColor" />}
                    {isRefilling ? 'Synchronizing Refill...' : 'Auto-Refill (10M)'}
                 </button>
                 <div className="flex-[0.5] p-6 bg-white/5 border border-white/10 rounded-[2rem] backdrop-blur-md flex items-center justify-center gap-3">
                    ...
                    <p className="text-[9px] font-black uppercase text-slate-400 leading-tight">System protocol available for instant liquidity injection.</p>
                 </div>
              </div>
           ...
           <div className="lg:col-span-1 space-y-6">
              <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-6">
                 <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2 border-b border-slate-50 pb-4">
                    <History size={14} className="text-blue-600" /> Recent Node Pulse
                 </h4>
                 <div className="space-y-4 max-h-[400px] overflow-y-auto custom-scrollbar">
                    ...
                    {state.ledger.filter(l => l.userId === currentUser.email).length === 0 && (
                      <p className="text-center py-10 text-[10px] font-black text-slate-300 uppercase italic">No pulse logs found.</p>
                    )}
                 </div>
              </div>
           </div>
           ...
           <div className="grid grid-cols-1 gap-4">
              {pendingTopups.map(req => (
                <div key={req.id} className="bg-white rounded-[2rem] p-8 border border-slate-100 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6 hover:shadow-md transition-all">
                   ...
                   <div className="flex gap-3">
                      ...
                      >
                         {isProcessing === req.id ? <RefreshCw className="animate-spin" size={14}/> : <CheckCircle size={14}/>}
                         Approve Topup
                      </button>
                   </div>
                </div>
              ...
              {pendingTopups.length === 0 && (
                <div className="p-20 text-center bg-white rounded-[2rem] border border-dashed border-slate-200">
                   <p className="text-slate-400 font-black uppercase tracking-widest text-xs">No pending topup requests in node registry.</p>
                </div>
              )}
           </div>
        </div>
           ...
           <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm">
              <div className="relative">
                 ...
                 <input 
                  ...
                  placeholder={`Search ${activeTab === 'personnel' ? 'colleagues' : 'subscribers'} to provision funds...`}
                  value={searchTerm}
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-[500] flex items-center justify-center p-4">
           <div className="bg-white rounded-[3rem] w-full max-w-lg shadow-2xl animate-in zoom-in duration-300 overflow-hidden border border-white/20 flex flex-col">
              <div className="px-10 py-8 bg-emerald-600 text-white flex justify-between items-center">
                 <div>
                    <h3 className="text-2xl font-black uppercase tracking-tighter italic">Provision Liquidity</h3>
              ...
              <div className="p-10 space-y-10">
                 <div>
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 block ml-1">Transfer Amount (Rs.)</label>
                    <div className="relative">

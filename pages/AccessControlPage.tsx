...
const AccessControlPage: React.FC<{ state: AppState }> = ({ state }) => {
  ...
  const handleSave = () => {
    ...
    if (editingStaff) {
      db.updateStaff(editingStaff.email, formData);
    } else {
            ...
            <div className="p-10 space-y-8 overflow-y-auto custom-scrollbar flex-1">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                   ...
                   <input type="email" className="w-full pl-14 pr-5 py-5 bg-slate-50 border-2 border-slate-100 rounded-2xl outline-none font-black text-slate-800 disabled:opacity-50" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} placeholder="id@netrecover.com" disabled={!!editingStaff} />
                </div>
              </div>

...
const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  const handleResetRequest = async (e: React.FormEvent) => {
     ...
     setTimeout(() => {
        ...
        if (userNode) {
          ...
          db.logNotification(userNode.id, 'info', 'Password Reset Initiated', `Reset code dispatched to registry node.`);
        } else {
          setError("IDENTITY_NOT_FOUND: No subscriber node matches this identifier.");
        }
     }, 1200);
  };
  ...
  const renderSignup = () => (
    <form onSubmit={handleSignup} className="space-y-4 animate-in slide-in-from-right duration-500" autoComplete="off">
       <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div className="space-y-1">
             <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Full Name</label>
             <input className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl font-bold text-xs" placeholder="e.g. John Doe" value={signupData.name} onChange={e => setSignupData({...signupData, name: e.target.value})} required autoComplete="off" />
          </div>
          <div className="space-y-1">
             <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Phone Number</label>
             <input className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl font-bold text-xs" placeholder="03XX-XXXXXXX" value={signupData.phone} onChange={e => setSignupData({...signupData, phone: e.target.value})} required autoComplete="off" />
          </div>
       </div>

       <div className="space-y-1">
          <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Email Address</label>
          <input className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl font-bold text-xs" type="email" placeholder="name@example.com" value={signupData.email} onChange={e => setSignupData({...signupData, email: e.target.value})} required autoComplete="off" />
       </div>
       ...
       <div className="space-y-1">
          ...
          <input className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl font-bold text-xs uppercase" placeholder="e.g. Gulshan, North Nazimabad" value={signupData.area} onChange={e => setSignupData({...signupData, area: e.target.value})} required autoComplete="off" />
       </div>
    ...
    <form onSubmit={handleResetFinalize} className="space-y-8 animate-in slide-in-from-right duration-500" autoComplete="off">
       ...
       <div className="space-y-6">
          <div className="space-y-2">
             ...
             <input className="w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl font-black text-center text-xl tracking-[0.5em] focus:border-indigo-600 outline-none" placeholder="XXXXXX" value={resetToken} onChange={e => setResetToken(e.target.value)} maxLength={6} required autoComplete="off" />
          </div>

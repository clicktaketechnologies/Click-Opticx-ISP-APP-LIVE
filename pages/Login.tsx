...
const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  const renderLogin = () => (
    <form onSubmit={handleLogin} className="space-y-5 animate-in fade-in duration-500" autoComplete="off">
      ...
      <div className="flex items-center justify-between px-1">
         ...
         <button type="button" onClick={() => setView('reset_request')} className="text-[10px] font-black text-indigo-600 uppercase tracking-widest hover:underline">Forgot?</button>
      </div>

      <button 
        type="submit"
        disabled={isProcessing}
        className="w-full bg-slate-950 text-white font-black py-5 rounded-[2rem] hover:bg-black shadow-2xl active:scale-[0.98] transition-all flex items-center justify-center gap-3 uppercase tracking-[0.2em] text-xs disabled:opacity-50 mt-2"
      >
      ...
      <div className="text-center pt-2">
         <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
            Don't have an account? <button type="button" onClick={() => setView('signup')} className="text-indigo-600 hover:underline ml-1 font-black">Sign Up</button>
         </p>
      </div>
    </form>
  );
  ...
  const renderSignup = () => (
    <form onSubmit={handleSignup} className="space-y-4 animate-in slide-in-from-right duration-500">
       ...
       <div className="space-y-1">
          ...
          <input className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl font-bold text-xs uppercase" placeholder="e.g. Gulshan-e-Iqbal" value={signupData.area} onChange={e => setSignupData({...signupData, area: e.target.value})} required />
       </div>
       ...
       <button 
        type="submit"
        disabled={isProcessing}
        className="w-full bg-indigo-600 text-white font-black py-4 rounded-[1.5rem] shadow-xl hover:bg-indigo-700 active:scale-95 transition-all flex items-center justify-center gap-2 uppercase tracking-widest text-[10px] mt-2"
       >
  ...
  const renderResetFinalize = () => (
    <form onSubmit={handleResetFinalize} className="space-y-6 animate-in slide-in-from-right duration-500">
       <div className="p-5 bg-emerald-50 border-2 border-emerald-100 rounded-2xl flex items-start gap-3">
  ...
  if (view === 'pending') {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-[3rem] shadow-2xl p-8 sm:p-12 text-center space-y-8 animate-in zoom-in duration-500 border border-slate-100">
           <div className="w-24 h-24 bg-emerald-50 text-emerald-600 rounded-[2.5rem] flex items-center justify-center mx-auto shadow-inner border-4 border-emerald-100 relative">
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4 py-12">
      <div className="max-w-md w-full bg-white rounded-[3rem] shadow-2xl p-8 sm:p-10 border border-slate-100 animate-in fade-in zoom-in duration-700 relative flex flex-col h-auto">
        ...
        <div className="flex flex-col items-center mb-8 pt-4">
          <div className="w-20 h-20 bg-slate-950 rounded-3xl flex items-center justify-center shadow-2xl mb-6 border-4 border-white overflow-hidden p-3 transform hover:rotate-3 transition-transform">

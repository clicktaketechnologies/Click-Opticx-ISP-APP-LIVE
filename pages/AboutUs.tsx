...
const AboutUs: React.FC<{ state: AppState }> = ({ state }) => {
  ...
  const handleSave = async () => {
    ...
    setTimeout(() => {
      setIsSaving(false);
      setIsEditing(false);
      db.logNotification('all', 'success', 'Identity Synced', 'Global branding and infrastructure nodes updated.');
    }, 800);
  };
      ...
      <div className="space-y-8 animate-in fade-in duration-500 pb-20 max-w-5xl mx-auto">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-6">
           <div className="space-y-1">
              ...
              <p className="text-slate-500 font-medium uppercase text-[10px] tracking-widest">Protocol: Global Branding & Infrastructure Dossier</p>
           </div>
        ...
        <div className="bg-white rounded-[3rem] p-10 border border-slate-100 shadow-sm p-10 space-y-12">
           <div className="space-y-8">
              <div className="flex items-center gap-3 pb-4 border-b border-slate-100">
                 ...
                 <h3 className="text-lg font-black uppercase italic tracking-tight">Corporate Identity</h3>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 italic">Business Trading Name</label>
                  <input 
                    type="text" 
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                 {[
                   { id: 'logoLight', label: 'Primary Logo (Light BG)', desc: 'Used in main header' },
                   { id: 'logoDark', label: 'Inverted Logo (Dark BG)', desc: 'Used in dark sidebars' },
                   { id: 'logoSquare', label: 'Square App Icon', desc: 'Used in mobile view' }
                 ].map(slot => (
           ...
           <div className="space-y-8 pt-8 border-t border-slate-100">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 italic">Target IP Address</label>
                  <input 
                    type="text" 
                    className="w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl font-black text-sm outline-none focus:border-indigo-600" 
                ...
                <div className="space-y-2">
                   <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 italic">DNS Propagation State</label>
                   <select 
                   ...
                   >
                      ...
                      <option value="ERROR">Protocol Error</option>
                   </select>
                </div>
              </div>
           </div>
           ...
           <div className="space-y-8 pt-8 border-t border-slate-100">
              <div className="flex items-center gap-3 pb-4 border-b border-slate-100">
                 ...
                 <h3 className="text-lg font-black uppercase italic tracking-tight">Dossier Content</h3>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                 <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 italic">Mission Protocol</label>
                 ...
                 <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 italic">Vision Archetype</label>
              ...
              <div className="space-y-2">
                 <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 italic">Corporate Story & History</label>
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-20 max-w-5xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
               <div className="flex items-center gap-8">
                  ...
                  <div>
                    ...
                    <p className="text-[12px] font-black text-indigo-400 uppercase tracking-[0.5em] mt-3 opacity-80 italic">Authority Infrastructure Dossier</p>
                  </div>
               </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-10 border-t border-white/5">
               <div className="p-10 bg-white/5 rounded-[2.5rem] backdrop-blur-xl border border-white/10 space-y-4 group hover:bg-white/10 transition-all hover:translate-y-[-4px]">
                  <div className="flex items-center gap-4 text-amber-400 mb-2">
                     ...
                     <h4 className="font-black uppercase tracking-[0.3em] text-xs">Vision Archetype</h4>
                  </div>
               ...
               <div className="p-10 bg-white/5 rounded-[2.5rem] backdrop-blur-xl border border-white/10 space-y-4 group hover:bg-white/10 transition-all hover:translate-y-[-4px]">
                  <div className="flex items-center gap-4 text-emerald-400 mb-2">
                     ...
                     <h4 className="font-black uppercase tracking-[0.3em] text-xs">Mission Protocol</h4>
                  </div>
      ...
      <div className="bg-white rounded-[3rem] p-10 border border-slate-100 shadow-sm space-y-8">
         <div className="flex items-center justify-between">
            <h3 className="text-[11px] font-black text-slate-400 uppercase tracking-[0.4em] flex items-center gap-3">
               <Network size={20} className="text-blue-600" /> Infrastructure Registry (DNS)
            </h3>
            <span className={`px-3 py-1 rounded-lg text-[8px] font-black uppercase tracking-widest border ${
              ...
              {state.settings.infrastructure.dnsStatus}
            </span>
         </div>
         <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100 flex items-center justify-between group hover:bg-white transition-all">
               ...
               <div className="text-right">
                  <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1">Target IP</p>
                  <p className="text-sm font-bold text-blue-600 font-mono">{state.settings.infrastructure.targetIP}</p>
               </div>
            </div>
            <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100 flex items-center justify-between group hover:bg-white transition-all">
               <div>
                  <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1">DNS Handshake</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
         <div className="lg:col-span-2 bg-white rounded-[3rem] p-12 border border-slate-100 shadow-sm space-y-8 relative overflow-hidden group">
            <h3 className="text-[11px] font-black text-slate-400 uppercase tracking-[0.4em] flex items-center gap-3">
               <History size={20} className="text-indigo-600" /> Organizational Narrative
            </h3>
         ...
         <div className="bg-indigo-600 rounded-[3rem] p-12 text-white relative overflow-hidden shadow-xl">
            <div className="relative z-10 space-y-10">
               <h3 className="text-[11px] font-black uppercase tracking-[0.4em] flex items-center gap-3 text-indigo-200">
                  <Star size={20} className="text-amber-400" /> Operational Values
               </h3>
      ...
      <div className="bg-white border-2 border-slate-100 rounded-[4rem] p-12 flex flex-col md:flex-row items-center justify-between gap-12 shadow-xl hover:shadow-2xl transition-all duration-500">
         <div className="flex flex-col md:flex-row items-center gap-8 text-center md:text-left">
            ...
            <div>
               <h4 className="text-3xl font-black text-slate-900 uppercase italic tracking-tighter">Certified Infrastructure</h4>
               <p className="text-[11px] text-slate-400 font-black uppercase tracking-[0.2em] mt-2">Registry Validated • ISO Synchronized Node • {formData.version}</p>
            </div>
         </div>
         <div className="flex flex-col gap-3 w-full md:w-auto">
            <button className="px-12 py-5 bg-slate-950 text-white rounded-3xl font-black text-[11px] uppercase tracking-[0.2em] hover:bg-black transition-all shadow-2xl active:scale-95 flex items-center justify-center gap-3">
               Verify System Integrity <ArrowRight size={16} />
            </button>
         </div>
      </div>
    </div>

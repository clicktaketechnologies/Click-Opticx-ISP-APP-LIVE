    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          ...
          <p className="text-slate-500 font-medium">Provision automated digital handshakes based on registry event triggers.</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 gap-4">
         ...
         {rules.length === 0 && (
           <div className="py-32 text-center bg-white rounded-[4rem] border-4 border-dashed border-slate-50 flex flex-col items-center">
              ...
              <h3 className="text-2xl font-black text-slate-800 uppercase tracking-tighter mb-1 italic">Rules Engine Idle</h3>
              <p className="text-slate-400 font-bold uppercase tracking-widest text-[9px]">Initialize an automation node to begin autonomous relay.</p>
           </div>
         )}
      </div>
      ...
      <div className="p-10 bg-slate-900 rounded-[3.5rem] text-white flex flex-col md:flex-row items-center justify-between gap-10 relative overflow-hidden shadow-2xl">
         <div className="relative z-10 max-w-xl space-y-4">
            <div className="flex items-center gap-4 text-amber-400">
               <ShieldAlert size={28} />
               <h3 className="text-2xl font-black uppercase tracking-tight italic leading-none">Security Threshold Node</h3>
            </div>
            <p className="text-sm font-bold text-slate-400 leading-relaxed uppercase opacity-80 italic">
               Automation rules operate via cron/event handshakes. Rule execution logic is isolated from core fiscal ledger manipulation for system safety.
            </p>
         </div>
           ...
           <div className="bg-white rounded-[3.5rem] w-full max-w-2xl shadow-2xl overflow-hidden border-[8px] border-slate-50 animate-in zoom-in duration-300 flex flex-col max-h-[90vh]">
              <div className="p-10 border-b bg-slate-50 flex justify-between items-center shrink-0">
                 <div className="flex items-center gap-5">
                    ...
                    <div>
                       <h3 className="text-2xl font-black uppercase italic tracking-tighter">Append Protocol</h3>
                       <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest">Automation Schema Initialization</p>
                    </div>
                 </div>
              ...
              <div className="p-10 space-y-8 flex-1 overflow-y-auto custom-scrollbar">
                 <div className="space-y-6">
                    <div className="space-y-2">
                       <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 italic">Protocol Label</label>
                       <input className="w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl font-black text-sm outline-none focus:border-amber-500 transition-all" placeholder="e.g. Near-Expiry Handshake" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
                    </div>
                    
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                       <div className="space-y-2">
                          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 italic">Event Trigger (WHEN)</label>
                          <select className="w-full p-4 bg-white border-2 border-slate-100 rounded-2xl font-black text-[11px] uppercase outline-none focus:border-amber-500" value={formData.trigger} onChange={e => setFormData({...formData, trigger: e.target.value as any})}>
                             <option value="Package_Expiry">Package Expiry Cycle</option>
                             <option value="Payment_Failed">Digital Payment Denial</option>
                             <option value="Emergency_Load_Active">Rescue Credit Engaged</option>
                             <option value="Outage_Detected">NOC Advisory Node</option>
                             <option value="Signup_Approved">Identity Initialized</option>
                          </select>
                       </div>
                       <div className="space-y-2">
                          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 italic">Logical Rule (DSL)</label>
                 ...
                 <div className="space-y-6 pt-6 border-t border-slate-50">
                    <h4 className="text-[10px] font-black text-slate-900 uppercase tracking-widest ml-1">Action Handshakes (THEN)</h4>
                         ...
                         <div key={idx} className="p-6 bg-slate-50 border-2 border-slate-100 rounded-[2.5rem] flex items-center justify-between">
                            <div className="flex items-center gap-4">
                               ...
                               <div className="space-y-1">
                                  <p className="text-[9px] font-black text-slate-400 uppercase">{action.type} Node</p>
                 ...
                 <div className="p-8 bg-amber-50 border border-amber-100 rounded-[2.5rem] flex items-start gap-4">
                    ...
                    <p className="text-[10px] text-amber-700 font-bold uppercase leading-relaxed tracking-tighter">
                       Rule activation triggers an immediate background listener handshake. Registry nodes will poll every 15 minutes for matching identity conditions.
                    </p>
                 </div>
              </div>

    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-20 max-w-5xl mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          ...
          <p className="text-slate-500 font-medium">Initialize real-time digital handshakes with subscriber mobile/web terminals.</p>
        </div>
        <div className="px-6 py-3 bg-white border border-slate-100 rounded-2xl shadow-sm flex items-center gap-3">
          <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div>
          <span className="text-[10px] font-black uppercase text-slate-700">PWA Gateway: Synced</span>
        </div>
      </div>
         ...
         <div className="lg:col-span-2 space-y-8">
            <div className="bg-white rounded-[3rem] p-10 border border-slate-100 shadow-sm space-y-10">
               <div className="flex bg-slate-50 p-1.5 rounded-2xl gap-1">
                  {['Individual', 'Segment', 'Global'].map((t: any) => (
                    <button
                      key={t}
                      onClick={() => { setTargetType(t); setTargetId(''); }}
                      className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${targetType === t ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-slate-600'}`}
                    >
                      {t}
                    </button>
                  ))}
               ...
               <div className="space-y-6">
                  ...
                  {targetType === 'Segment' && (
                    <div className="space-y-2 animate-in slide-in-from-top-2">
                       <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 italic">Identity Segment Node</label>
                       <select className="w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl font-black text-sm outline-none focus:border-indigo-600" value={targetId} onChange={e => setTargetId(e.target.value)}>
                          <option value="">Choose Segment...</option>
                          {state.audienceSegments.map(s => <option key={s.id} value={s.id}>{s.name} ({s.subscriberCount} Nodes)</option>)}
                       </select>
                    </div>
                  )}
                  ...
                  <div className="space-y-6">
                     <div className="space-y-2">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 italic">Relay Title</label>
                     ...
                     <div className="space-y-2">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 italic">Payload Message</label>
                        <textarea className="w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl font-bold text-xs h-32 resize-none outline-none focus:border-indigo-600 uppercase" placeholder="Brief technical advisory..." value={message} onChange={e => setMessage(e.target.value)} />
                     </div>
                  </div>
                  ...
                  <div className="flex justify-between items-center bg-slate-50 p-6 rounded-[2rem] border border-slate-100">
                     <div className="flex items-center gap-4">
                        ...
                        <div>
                           <h4 className="text-xs font-black uppercase text-slate-900">Priority Tier</h4>
                           <p className="text-[9px] text-slate-400 font-bold uppercase">Critical nodes bypass DND settings</p>
                        </div>
                     </div>
                     <button 
         ...
         <div className="space-y-6">
            <div className="bg-slate-900 rounded-[2.5rem] p-10 text-white space-y-8 shadow-2xl relative overflow-hidden flex flex-col justify-between h-[300px]">
               <div className="relative z-10 space-y-4">
                  <h4 className="text-xs font-black uppercase tracking-widest text-indigo-400">Registry Pulse</h4>
                  <div className="grid grid-cols-1 gap-4">
                     <div className="p-4 bg-white/5 border border-white/10 rounded-2xl">
                        <p className="text-[9px] font-black text-slate-500 uppercase mb-1 italic">Deliverability Rank</p>
            ...
            {success && (
              <div className="p-8 bg-emerald-50 border-2 border-emerald-100 rounded-[2.5rem] flex items-center gap-6 animate-in zoom-in duration-300">
                 ...
                 <div>
                    <h4 className="text-sm font-black text-emerald-900 uppercase italic">Dispatch Verified</h4>
                    <p className="text-[9px] text-emerald-700 font-bold uppercase tracking-widest leading-tight">Payload successfully published to regional PWA nodes.</p>
                 </div>
              </div>
            )}
            ...
            <div className="p-8 bg-blue-50 border border-blue-100 rounded-[2.5rem] space-y-6">
               <div className="flex items-center gap-3">
                  ...
                  <h4 className="text-xs font-black uppercase tracking-widest text-blue-900">Push Guidelines</h4>
               </div>
               <ul className="space-y-4">
                  {[
                    'Manual push relays are limited to 3 per node/day.',
                    'Global broadcasts require SuperAdmin log authorization.',
                    'Payloads must not exceed 240 registry bytes.'
                  ].map((rule, i) => (

    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          ...
          <p className="text-slate-500 font-medium">Build logical identity clusters for targeted communication handshakes.</p>
        </div>
        <button 
        ...
        >
          <Plus size={18} /> Provision Segment
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
           ...
           <div key={seg.id} className="bg-white rounded-[3rem] p-10 border border-slate-100 shadow-sm hover:shadow-xl transition-all group relative overflow-hidden flex flex-col h-full">
              ...
              <div className="mt-auto space-y-4 relative z-10 pt-6 border-t border-slate-50">
                 ...
                 <div className="flex gap-2">
                    <button onClick={() => { setFormData(seg); setIsModalOpen(true); }} className="flex-1 py-3 bg-slate-50 text-slate-500 rounded-xl font-black text-[9px] uppercase hover:bg-indigo-50 hover:text-indigo-600 transition-all border border-slate-100">Audit Protocol</button>
         ...
         {filteredSegments.length === 0 && (
           <div className="col-span-full py-32 text-center bg-white rounded-[4rem] border-4 border-dashed border-slate-50 flex flex-col items-center">
              ...
              <h3 className="text-2xl font-black text-slate-800 uppercase tracking-tighter mb-1 italic">Segment Map Dormant</h3>
              <p className="text-slate-400 font-bold uppercase tracking-widest text-[9px]">Establish your first audience node to begin targeting.</p>
           </div>
         )}
      </div>
           ...
           <div className="bg-white rounded-[3.5rem] w-full max-w-lg shadow-2xl overflow-hidden border-[8px] border-slate-50 animate-in zoom-in duration-300 flex flex-col max-h-[90vh]">
              <div className="p-10 border-b bg-slate-50 flex justify-between items-center shrink-0">
                 <div className="flex items-center gap-5">
                    ...
                    <div>
                       <h3 className="text-2xl font-black uppercase italic tracking-tighter">Segment Config</h3>
                       <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest">Logic Filtering Handshake</p>
                    </div>
                 </div>
              ...
              <div className="p-10 space-y-8 flex-1 overflow-y-auto custom-scrollbar">
                 <div className="space-y-6">
                    <div className="space-y-2">
                       <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 italic">Logical Name</label>
                       <input className="w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl font-black text-sm outline-none focus:border-blue-600 transition-all" placeholder="e.g. Delinquent Nodes (KHI)" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
                    </div>
                    <div className="space-y-2">
                       <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 italic">Segment Objective</label>
                       <textarea className="w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl font-bold text-xs h-24 resize-none outline-none focus:border-blue-600 transition-all uppercase" placeholder="Describe the targeting logic..." value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} />
                    </div>
                 </div>
                    ...
                    <div className="grid grid-cols-1 gap-4">
                       <div className="p-6 bg-slate-50 rounded-[2rem] border-2 border-slate-100 space-y-4">
                          <div className="flex justify-between items-center">
                             <span className="text-[9px] font-black uppercase text-slate-500">Registry Status</span>
                             <select 
                 ...
                 <div className="p-8 bg-blue-50 border border-blue-100 rounded-[2.5rem] flex items-center justify-between shadow-inner">
                    <div className="flex items-center gap-4">
                       ...
                       <div>
                          <p className="text-[10px] font-black text-blue-900 uppercase">Estimated Pulse</p>
                          <p className="text-xs font-black text-blue-700 italic">Approx. {state.users.length} Nodes scanned</p>
                       </div>
                    </div>
                 </div>
              </div>

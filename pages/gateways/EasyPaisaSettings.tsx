...
import { 
  ...
  Database, Fingerprint, Lock, Eye, EyeOff, Activity, ShieldAlert
} from 'lucide-react';
...
const EasyPaisaSettings: React.FC<Props> = ({ state, onBack }) => {
  const gateway = state.settings.paymentGateways.find(g => g.id === 'easypaisa');
  ...
  if (!gateway) {
    return (
      <div className="flex flex-col items-center justify-center p-20 text-center space-y-6 bg-white rounded-[3rem] border border-slate-100 shadow-sm animate-in zoom-in">
         ...
         <h3 className="text-xl font-black text-slate-900 uppercase italic">EasyPaisa Node Not Found</h3>
  ...
  const handleSave = async () => {
    setIsSaving(true);
    await db.updateGatewayConfig('easypaisa', formData);
    setTimeout(() => {
      setIsSaving(false);
      db.logNotification('all', 'success', 'EasyPaisa Synced', 'Mobile payment node credentials published.');
    }, 800);
  };
  ...
  return (
    <div className="space-y-8 animate-in slide-in-from-right-4 duration-500 pb-20 max-w-4xl mx-auto">
      <div className="flex items-center gap-6">
         ...
         <div>
            <h2 className="text-3xl font-black text-slate-800 tracking-tight italic uppercase">EasyPaisa Hub</h2>
            <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest mt-1">Mobile Handshake Registry Node</p>
         </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-white rounded-[3rem] p-10 border border-slate-100 shadow-xl space-y-10">
               <div className="space-y-6">
                  <div className="space-y-2">
                     <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 italic">Authorized Store ID</label>
                     <input className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl font-black text-sm" value={formData.config.storeId} onChange={e => updateConfig('storeId', e.target.value)} />
                  </div>
                  <div className="space-y-2">
         ...
         <div className="space-y-6">
            <div className="bg-slate-900 rounded-[2.5rem] p-10 text-white relative overflow-hidden shadow-2xl">
               ...
               <Smartphone className="absolute -right-8 -bottom-8 opacity-5 scale-150" size={140} />
            </div>
         </div>
      </div>
    </div>

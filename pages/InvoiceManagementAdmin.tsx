...
const InvoiceManagementAdmin: React.FC<Props> = ({ state, onNavigate }) => {
  ...
  const handleBulkReminder = async () => {
    ...
    setSelectedIds(new Set());
    // Fixed: Added 'all' as the first argument (targetId) to db.logNotification
    db.logNotification('all', 'info', 'Bulk Reminder', 'Dispatched payment alerts to selected subscriber nodes.');
  };

  const getStatusStyle = (status: PaymentStatus) => {
    switch(status) {
      case PaymentStatus.PAID: return 'bg-emerald-50 text-emerald-600 border-emerald-100';
        ...
        <div className="bg-slate-900 rounded-[2.5rem] p-10 text-white flex flex-col md:flex-row items-center justify-between shadow-2xl relative overflow-hidden border border-white/5">
           <div className="relative z-10 flex items-center gap-8">
              ...
              <div className="space-y-1">
                 ...
                 <p className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.4em] italic opacity-80">Official Fiscal Registry • Node Authority</p>
        ...
        <div className="bg-slate-900 text-white p-6 rounded-[2.5rem] flex flex-col md:flex-row items-center justify-between gap-6 animate-in slide-in-from-top-4 shadow-2xl sticky top-20 z-[110]">
           ...
           <div className="flex gap-3">
              ...
              <button 
                ...
                disabled={isProcessing === 'bulk'}
                className="flex items-center gap-2 px-6 py-3 bg-emerald-600 hover:bg-emerald-700 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shadow-xl shadow-emerald-500/20"
              >
      ...
      <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm flex flex-col lg:flex-row gap-4 items-center">
         ...
         {activeView === 'invoices' && (
           <div className="flex gap-2 w-full lg:w-auto">
              <select 
                className="px-6 py-4 bg-slate-100 border-none rounded-2xl text-[10px] font-black uppercase tracking-widest outline-none focus:ring-4 focus:ring-indigo-500/10 transition-all"
                value={statusFilter}
                ...
                <tbody className="divide-y divide-slate-50">
                    {filteredInvoices.length === 0 ? (
                      ...
                      filteredInvoices.map(inv => {
                        const user = userMap.get(inv.userId);
                        return (
                          <tr key={inv.id} className={`hover:bg-slate-50 transition-colors group ${selectedIds.has(inv.id) ? 'bg-indigo-50/50' : ''}`}>
                            <td className="px-8 py-6">
                               <button onClick={() => toggleSelect(inv.id)} className="p-1 text-slate-200 hover:text-indigo-600">
                                  {selectedIds.has(inv.id) ? <CheckSquare size={18} /> : <Square size={18} />}
                               </button>
                            </td>
                            <td className="px-8 py-6">
                              <div className="flex items-center gap-3">
                                  <div className="w-8 h-8 bg-indigo-50 text-indigo-600 rounded-lg flex items-center justify-center border shadow-inner">
                                    <FileText size={16}/>
                                  </div>
                            ...
                            <td className="px-8 py-6">
                              <div className="flex flex-col">
                                  <span className="font-black text-slate-800 uppercase tracking-tight text-sm leading-none mb-1 group-hover:text-indigo-600 transition-colors">{inv.userName}</span>
                                  <div className="flex items-center gap-2">
                                     <span className="text-[9px] text-slate-400 font-bold uppercase tracking-widest">{inv.userId}</span>
                                     {user && <span className={`text-[8px] font-black uppercase px-1.5 py-0.5 rounded ${user.status === 'Active' ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}`}>{user.status}</span>}
                                  </div>
                              </div>
                            </td>
                            ...
                            <td className="px-8 py-6 text-right">
                              <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                  <button 
                                    className="p-3 bg-white border border-slate-200 text-slate-400 hover:text-emerald-600 rounded-xl shadow-sm transition-all" 
              ...
              <table className="w-full text-left min-w-[1000px]">
                <thead className="bg-slate-50 border-b border-slate-100">
                    <tr>
                      ...
                      <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Connection Node</th>
                    ...
                    {filteredClients.length === 0 ? (
                      ...
                      filteredClients.map(u => (
                        <tr key={u.id} className="hover:bg-slate-50 transition-colors group">
                          <td className="px-8 py-6">
                            <div className="flex items-center gap-4">
                                <div className="w-12 h-12 bg-slate-50 rounded-2xl flex items-center justify-center text-slate-400 group-hover:bg-indigo-50 group-hover:text-indigo-600 transition-all shadow-inner border border-slate-100">
                                  <UserCircle size={28} />
                                </div>
                                <div>
                          ...
                          <td className="px-8 py-6 text-right">
                            <span className={`text-sm font-black italic tracking-tighter ${u.balance > 0 ? 'text-red-600' : 'text-emerald-600'}`}>

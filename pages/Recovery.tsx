      ...
      <div className="space-y-8 animate-in fade-in duration-500 pb-20">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 w-full">
          <div className="hidden md:block">
            ...
            <p className="text-slate-500 font-bold mt-2 uppercase text-[clamp(0.5rem,1.5vw,0.6rem)] tracking-[0.3em] italic border-l-4 border-blue-500 pl-4">Operational Console for Suspended & Unpaid Registry</p>
          </div>
          <div className="flex flex-wrap gap-3 w-full md:w-auto shrink-0">
            <button
              onClick={() => handleSelectionShortcut('Clear')}
              disabled={selectedUsers.size === 0}
              className={`flex-1 md:flex-none flex items-center justify-center gap-2 px-5 py-3 bg-white border border-slate-200 text-slate-600 rounded-xl font-black text-[9px] uppercase tracking-widest hover:bg-slate-50 shadow-sm transition-all active:scale-95 text-center ${selectedUsers.size === 0 ? 'opacity-50 cursor-not-allowed' : ''}`}
            >
              <X size={14} /> Deselect ({selectedUsers.size})
            </button>
            <button
              onClick={() => alert("Printing recovery list...")}
              className="flex-1 md:flex-none flex items-center justify-center gap-2 px-5 py-3 bg-white border border-slate-200 text-slate-600 rounded-xl font-black text-[9px] uppercase tracking-widest hover:bg-slate-50 shadow-sm transition-all active:scale-95 text-center"
            >
            ...
            <div className="flex items-center gap-1 p-1 bg-slate-50 rounded-xl border border-slate-100">
              {(['All', 'Today', '3Days', '7Days'] as const).map(f => (
                <button
                  ...
                  className={`px-4 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${dateFilter === f ? 'bg-white text-blue-600 shadow-sm border border-blue-100' : 'text-slate-400 hover:text-slate-600'
                    }`}
                >

...
import { db } from '../db';
import { startCall, stopCall } from '../utils/vapi';
import {
                  ...
                  <div className="bg-white rounded-[2.5rem] p-8 text-slate-900 border border-slate-100 shadow-sm flex flex-col justify-between overflow-hidden relative">
                     <div className="relative z-10 space-y-6">
                        ...
                        <div className="space-y-4">
                           {[
                              { user: 'Ahmed Ali', issue: 'Internet Down', conf: 52 },
                              { user: 'Sara Khan', issue: 'Billing Query', conf: 94 }
                           ].map(call => (
         ...
         {activeTab === 'personality' && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in slide-in-from-right-4 duration-500">
               ...
               <div className="flex flex-col gap-6">
                  ...
                  <button onClick={() => startCall(state.settings.aiCallConfig.assistantId || 'aac4803d-82c8-47c3-9831-7557d1904128').then(res => res.success ? alert('Vapi Handshake Initiated') : alert('Failed to start Vapi: ' + JSON.stringify(res.error)))} className="w-full py-8 bg-indigo-600 text-white rounded-[2.5rem] font-black text-xs uppercase tracking-[0.3em] shadow-xl hover:bg-indigo-700 active:scale-95 transition-all">Publish Vocal Transformation</button>
               </div>
            </div>
         )}
               ...
               <div className="bg-white rounded-[4rem] w-full max-w-xl shadow-2xl overflow-hidden border-[10px] border-indigo-600 flex flex-col">
                  ...
                  <div className="p-12 space-y-10">
                     ...
                     <div className="flex gap-4">
                        ...
                        <button
                           onClick={() => {
                              startCall(state.settings.aiCallConfig.assistantId || 'aac4803d-82c8-47c3-9831-7557d1904128');
                              alert("COMM_CHANNEL_ESTABLISHED: AI Voice Muted. Human agent is live.");
                           }}

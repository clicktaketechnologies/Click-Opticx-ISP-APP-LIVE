...
import {
  ...
  Trash2, Edit3, CheckCircle, Ban, X, Globe, SignalHigh, RefreshCw, Activity
} from 'lucide-react';
...
const ConnectedDevices: React.FC<{ user: ISPUser }> = ({ user }) => {
  ...
  const handleTest = async (id: string) => {
    setLoadingIds(prev => new Set(prev).add(id));
    const result = await db.testDeviceConnection(id);
    setLoadingIds(prev => { const n = new Set(prev); n.delete(id); return n; });

    if (result) {
      db.logNotification(user.id, 'success', 'Node Pulse Verified', 'Device handshake acknowledged by core switch.');
    } else {
      db.logNotification(user.id, 'warning', 'Node Handshake Failed', 'Device is unreachable in the local registry.');
    }
  };
        ...
        {devices.map(dev => (
          <div key={dev.id} className={`bg-white p-6 rounded-[2.5rem] border-2 transition-all group ${dev.isBlocked ? 'border-rose-100 bg-rose-50/20' : 'border-slate-50 hover:border-indigo-100 hover:shadow-xl shadow-sm'}`}>
            ...
            <div className="mt-6 flex gap-3">
              ...
              >
                {loadingIds.has(dev.id) ? <RefreshCw className="animate-spin" size={14} /> : (dev.isBlocked ? <Shield size={14} /> : <Ban size={14} />)}
                {dev.isBlocked ? 'Authorize Access' : 'Restrict Node'}
              </button>
              <button
                onClick={() => handleTest(dev.id)}
                disabled={loadingIds.has(dev.id) || dev.isBlocked}
                className={`px-6 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all active:scale-95 shadow-sm flex items-center justify-center gap-2 bg-slate-100 text-slate-500 hover:bg-indigo-600 hover:text-white disabled:opacity-30`}
              >
                {loadingIds.has(dev.id) ? <RefreshCw className="animate-spin" size={14} /> : <Activity size={14} />}
                Test Link
              </button>
            </div>
          </div>

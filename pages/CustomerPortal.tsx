   ...
   return (
      <div className="space-y-8 animate-in fade-in duration-700">
         ...
         <div className="relative group">
            <div className="absolute -inset-1 bg-gradient-to-r from-blue-600 to-blue-600 rounded-[2.5rem] blur opacity-5 group-hover:opacity-10 transition duration-1000"></div>
                     ...
                     <div className="relative p-6 bg-white border border-slate-100 rounded-3xl hover:border-blue-500 transition-all text-left flex flex-col justify-between gap-6 h-full shadow-sm hover:shadow-2xl hover:shadow-blue-500/10 active:scale-[0.98]">
                        <div className="flex items-start justify-between">
                           ...
                           <div className="text-right">
                              ...
                              <p className={`text-[8px] font-black mt-1 px-2 py-0.5 rounded-full uppercase inline-block ${u.status === 'Active' ? 'bg-green-100 text-green-600' : 'bg-rose-100 text-rose-600'}`}>{u.status}</p>
                           </div>
                        </div>
                        <div>
         ...
         {selectedUser && (
            <div className="space-y-8 animate-in zoom-in duration-500">
               ...
               <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                     ...
                     <div className="relative z-10 h-full flex flex-col justify-between">
                        <div>
                           <div className="flex justify-between items-start mb-8">
                              <div className="w-20 h-20 bg-white/5 backdrop-blur-3xl rounded-[2rem] flex items-center justify-center text-green-400 border border-white/10 shadow-2xl relative overflow-hidden">
                           ...
                           <div className="flex items-center gap-3 mb-10">
                              <span className="text-blue-400 text-xs font-black uppercase tracking-widest">{selectedUser.connectionId}</span>
                              <span className="w-1 h-1 rounded-full bg-slate-700"></span>
                              <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded-md ${selectedUser.status === 'Active' ? 'bg-green-500/20 text-green-400' : 'bg-rose-500/20 text-rose-400'}`}>
                                 {selectedUser.status}
                              </span>
                           </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-6 pt-6 border-t border-white/5">
                           <div className="space-y-1">
                              ...
                              <p className="text-xs font-black flex items-center gap-2"><Globe size={12} className="text-blue-400" /> {selectedUser.macIp || 'DYNAMIC-IP'}</p>
                           </div>
                           <div className="space-y-1">
                              <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest">Service Node</p>
                              <p className="text-xs font-black flex items-center gap-2"><MapPin size={12} className="text-blue-400" /> {selectedUser.area}</p>
                           </div>
                           <div className="space-y-1 hidden md:block">
                              <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest">Contact Link</p>
                              <p className="text-xs font-black flex items-center gap-2"><Smartphone size={12} className="text-blue-400" /> {selectedUser.phone}</p>
                           </div>
                        </div>
                     </div>
                     <Activity className="absolute -right-20 -bottom-20 opacity-[0.03] text-blue-500" size={400} />
                  </div>
                     ...
                     <div>
                        ...
                        <div className="space-y-3">
                           <button 
                              ...
                              className={`w-full p-4 rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-3 transition-all shadow-lg active:scale-95 ${selectedUser.status === UserStatus.SUSPENDED ? 'bg-green-600 text-white hover:bg-green-700' : 'bg-slate-950 text-white hover:bg-black'}`}
                           >
                           ...
                           <button
                              ...
                              disabled={isProcessing}
                              className="w-full p-4 bg-blue-50 text-blue-600 border border-blue-100 rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-3 hover:bg-blue-100 active:scale-95 transition-all"
                           >
                  ...
                  {/* Health Metrics */}
                  <div className="bg-blue-600 rounded-[3rem] p-10 text-white shadow-xl shadow-blue-600/20 relative overflow-hidden">
                     <div className="relative z-10 flex flex-col h-full justify-between">
                        <div>
                           <p className="text-[10px] font-black text-blue-200 uppercase tracking-widest mb-6">Service Health</p>
                           <div className="space-y-2">
                              <div className="flex items-end justify-between">
                                 <span className="text-4xl font-black italic">99.8%</span>
                                 <span className="text-[10px] font-black text-green-400 uppercase">Uptime</span>
                              </div>
                              <div className="h-2 w-full bg-white/10 rounded-full overflow-hidden">
                                 <div className="h-full w-[99.8%] bg-green-400 shadow-[0_0_10px_#32d583]"></div>
                              </div>
                           </div>
                        </div>
                        <div className="bg-white/10 backdrop-blur-xl p-4 rounded-2xl border border-white/10 mt-8">
                           <p className="text-[8px] font-black text-blue-100 uppercase mb-2">Registry Payload</p>
                           <div className="flex items-center gap-2">
                              <HardDrive size={14} className="text-blue-200" />
                     ...
                     <div className="bg-white rounded-[3rem] p-10 border border-slate-100 shadow-xl shadow-slate-900/5 space-y-8">
                        <div className="flex items-center justify-between">
                           <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                              <CreditCard size={14} className="text-blue-500" /> Fiscal Registry
                           </h4>
                        ...
                        <div className="space-y-4">
                           <div className="p-8 bg-slate-50 rounded-[2.5rem] border border-slate-100 relative overflow-hidden group">
                              ...
                              <div className="flex items-end gap-2">
                                 <p className={`text-4xl font-black italic tracking-tighter ${selectedUser.balance > 0 ? 'text-rose-600' : 'text-green-600'}`}>
                                    {state.settings.currency} {(selectedUser.balance || 0).toLocaleString()}
                                 </p>
                                 <div className="mb-2">
                           ...
                           <div className="p-8 bg-blue-50/50 rounded-[2.5rem] border border-blue-100/50">
                              <p className="text-[10px] font-black text-blue-400 uppercase tracking-widest mb-2">Provisioned Package</p>
                              <div className="flex items-center justify-between">
                                 <p className="text-lg font-black text-blue-950 uppercase italic tracking-tight">
                        ...
                        <div className="pt-6">
                           ...
                           <div className="space-y-2">
                              {[1, 2].map(i => (
                                 <div key={i} className="flex items-center gap-3 text-[10px] text-slate-600 font-bold uppercase py-1 border-l-2 border-blue-500 pl-4">
                  ...
                  <div className="xl:col-span-2 bg-white rounded-[4rem] border border-slate-100 shadow-2xl overflow-hidden flex flex-col min-h-[700px]">
                     ...
                     <div className="flex-1 p-12 overflow-y-auto custom-scrollbar bg-white">
                        <div className="space-y-16">
                                          ...
                                          <tr key={inv.id} className="hover:bg-white transition-all group">
                                             ...
                                             <td className="px-8 py-6">
                                                <div className="flex items-center gap-3">
                                                   <div className="w-8 h-8 rounded-lg bg-blue-50 flex items-center justify-center text-blue-400 group-hover:bg-blue-600 group-hover:text-white transition-all">
                                                      <Receipt size={14} />
                                                   </div>
                                                   <div>
                                             ...
                                             <td className="px-8 py-6 text-center">
                                                <span className={`text-[9px] font-black px-3 py-1 rounded-lg uppercase tracking-widest ${inv.status === 'Paid' ? 'bg-green-100 text-green-700' : 'bg-rose-100 text-rose-700'}`}>
                                                   {inv.status}
                                                </span>
                                             </td>
                           ...
                           <div className="space-y-8">
                              <h4 className="text-xs font-black text-slate-900 uppercase tracking-widest flex items-center gap-2">
                                 <Wallet size={16} className="text-green-500" /> Fiscal Audit Trail (Ledger)
                              </h4>
                              <div className="grid gap-4">
                                 {userLedger.map(entry => (
                                    <div key={entry.id} className="group p-8 bg-white border border-slate-100 rounded-[2rem] flex items-center justify-between hover:shadow-2xl hover:shadow-slate-200 transition-all border-l-4 hover:border-l-blue-500 relative overflow-hidden">
                                       <div className="flex items-center gap-6 relative z-10">
                                          <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-lg transition-transform group-hover:scale-110 ${entry.type === LedgerType.DEBIT ? 'bg-rose-50 text-rose-500 shadow-rose-500/10' : 'bg-green-50 text-green-500 shadow-green-500/10'}`}>
                                       ...
                                       <div className="text-right relative z-10">
                                          <p className={`text-xl font-black italic tracking-tighter ${entry.type === LedgerType.DEBIT ? 'text-rose-600' : 'text-green-600'}`}>
                                             {entry.type === LedgerType.DEBIT ? '-' : '+'} {state.settings.currency}{(entry.amount || 0).toLocaleString()}
                                          </p>

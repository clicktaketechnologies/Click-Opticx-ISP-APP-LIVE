...
const SubscriberApp: React.FC<{ state: AppState; user: ISPUser; onLogout: () => void }> = ({ state, user, onLogout }) => {
   ...
   const isPageEnabled = (id: string) => {
      const corePages = [
         'home', 'profile', 'wallet', 'packages', 'billing', 'support', 'notifs',
         'ai-home', 'ai-insights', 'ai-network', 'ai-risk', 'ai-suggestions', 'aichat',
         'emergency', 'emergency-request', 'emergency-history', 'ai-voice-call', 'legal'
      ];
      if (corePages.includes(id)) return true;

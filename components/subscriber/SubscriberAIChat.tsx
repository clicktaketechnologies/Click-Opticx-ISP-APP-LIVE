...
const SubscriberAIChat: React.FC<{ user: ISPUser, state: AppState }> = ({ user, state }) => {
   ...
   const handleSend = useCallback(async () => {
      ...
      STYLE: Industrial, professional, uppercase where appropriate.`;
         ...
         const aiText = response.text || "PROTOCOL_ERROR: SYNC_LOST";
         ...
         const errorMsg = "AI CORE OFFLINE: Handshake Interrupted.";
         setAiError(errorMsg);
      ...
      <div className="flex flex-col h-full bg-white rounded-[2.5rem] border border-slate-100 shadow-2xl overflow-hidden animate-in zoom-in duration-300">
         <header className="p-8 bg-slate-900 text-white flex justify-between items-center shrink-0 relative overflow-hidden">
            <div className="relative z-10 flex items-center gap-4">
               ...
               <div>
                  <h3 className="text-xl font-black uppercase tracking-tight italic leading-none">Click Opticx Hybrid AI</h3>
         ...
         <div className="p-6 bg-white border-t border-slate-100 shrink-0">
            <div className="relative flex items-center gap-3">
               ...
               <button
                  ...
                  className="absolute right-2 p-4 bg-indigo-600 text-white rounded-xl shadow-lg active:scale-90 transition-all disabled:opacity-50"
               >
                  <Send size={20} />
               </button>
            </div>

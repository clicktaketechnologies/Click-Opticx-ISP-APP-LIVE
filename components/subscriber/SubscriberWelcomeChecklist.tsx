...
const SubscriberWelcomeChecklist: React.FC<Props> = ({ user, onComplete }) => {
  ...
  const submitKYCProtocol = async () => {
    ...
    try {
      ...
      alert("KYC Registry Uplink Failure.");
    } finally {
  ...
  const handlePasswordChange = async (e: React.MouseEvent) => {
    ...
    if (!newPass || newPass.length < 6) {
      alert("Security Protocol: Access secret must be at least 6 characters.");
      return;
    }
    ...
    try {
      ...
      if (res.success) {
        ...
        setPassView('confirm');
        setActiveSubStep(null);
        db.logNotification(user.id, 'success', 'Secret Rotated', 'Your access secret has been updated in the node registry.');
      } else {
        alert("Registry Denial: Handshake failed. Please try again.");
      }
    } catch (err) {
      console.error("Password rotation error:", err);
      alert("Credential Rotation Handshake Failed.");
    } finally {
      setIsProcessing(false);
    }
  };
    ...
    switch (activeSubStep) {
        ...
        return (
          <div className="space-y-8 animate-in slide-in-from-right duration-300">
            <div className="space-y-2">
               <h4 className="text-xl font-black uppercase italic tracking-tight text-slate-800">Identity Handshake</h4>
               <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest leading-relaxed">
                  Provision valid documentation to initialize your node security level.
               </p>
            </div>
      ...
      case 'Setup Secure Password':
        return (
          <div className="space-y-8 animate-in slide-in-from-right duration-300">
             <div className="space-y-2">
               <h4 className="text-xl font-black uppercase italic tracking-tight text-slate-800">Access Secret Validation</h4>
               <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest leading-relaxed">
                  Verify or rotate your system access secret.
               </p>
            </div>
            ...
            {passView === 'confirm' ? (
              <div className="space-y-6">
                <div className="p-8 bg-slate-900 rounded-[2.5rem] text-center border-4 border-slate-800 shadow-inner">
                   <p className="text-[9px] font-black text-slate-500 uppercase tracking-[0.4em] mb-4">Current Secret</p>
                   <h2 className="text-2xl font-black text-indigo-400 italic tracking-tighter">{(user.password || '******').toUpperCase()}</h2>
                </div>
                <div className="grid grid-cols-2 gap-4">
                   <button onClick={() => setPassView('change')} className="py-5 bg-slate-100 text-slate-500 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-slate-200">Change Token</button>
                   <button onClick={handlePasswordConfirm} className="py-5 bg-emerald-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-lg active:scale-95">Verify Existing</button>
                </div>
                <button 
                  onClick={() => setActiveSubStep(null)}
              ...
              <div className="space-y-6">
                 <div className="space-y-2">
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">New Node Secret (Min 6 Characters)</label>
      ...
      case 'Link Mobile Node':
        return (
          <div className="space-y-8 animate-in slide-in-from-right duration-300">
             <div className="space-y-2">
               <h4 className="text-xl font-black uppercase italic tracking-tight text-slate-800">Identity Mapping Confirmation</h4>
               <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest leading-relaxed">
                  Validate your network handle aliases to finalize hardware handshake.
               </p>
            </div>

            <div className="space-y-3">
               <div className="p-6 bg-slate-50 border border-slate-100 rounded-3xl flex items-center justify-between shadow-inner">
                  <div><p className="text-[8px] font-black text-slate-400 uppercase">App Identity</p><p className="text-sm font-black text-slate-900 uppercase italic">{user.username || user.connectionId}</p></div>
                  <UserCheck className="text-emerald-500" size={20} />
               </div>
               <div className="p-6 bg-slate-50 border border-slate-100 rounded-3xl flex items-center justify-between shadow-inner">
                  <div><p className="text-[8px] font-black text-slate-400 uppercase">PPPoE Logic</p><p className="text-sm font-black text-slate-900 uppercase italic">{user.pppoeId || 'P-RECOVERY-NODE'}</p></div>
            ...
            <div className="flex flex-col gap-3">
              <button 
                onClick={() => { markStepComplete('Link Mobile Node'); setActiveSubStep(null); }}
              ...
              >
                <Zap size={18} fill="currentColor"/> Commit Mapping Handshake
              </button>
              <button 
                onClick={() => setActiveSubStep(null)}
                className="w-full py-4 text-slate-400 font-black text-[10px] uppercase tracking-widest hover:text-slate-600 flex items-center justify-center gap-2"
          ...
          <div className="space-y-8">
            <div className="p-8 bg-slate-50 border border-slate-100 rounded-[2.5rem] flex items-start gap-4 shadow-inner">
               ...
               <p className="text-[10px] text-slate-600 font-bold leading-relaxed uppercase">
                  Your node dossier has been authorized. Complete these protocols to fully activate your dashboard.
               </p>
            </div>
            ...
            <div className="flex gap-4">
               ...
               <div className="flex-1 p-5 bg-slate-50 rounded-[2rem] border border-slate-100 flex flex-col items-center gap-2 text-center opacity-60">
                  ...
                  <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest">Global Relay</p>
               </div>
            </div>
          </div>
        );
      ...
      <div className="bg-white w-full max-w-lg rounded-[3.5rem] shadow-2xl overflow-hidden border-[8px] border-slate-100 flex flex-col max-h-[90vh] animate-in zoom-in duration-300 relative">
        <div className="p-10 bg-slate-900 text-white relative overflow-hidden shrink-0">
           <div className="relative z-10 flex justify-between items-center pr-10">
              <div className="space-y-2">
                 <h3 className="text-3xl font-black italic tracking-tighter uppercase leading-none">AUTHORIZATION</h3>
                 <p className="text-emerald-400 text-[10px] font-black uppercase tracking-[0.4em]">Initial Node Provisioning</p>
              </div>
           </div>
        ...
        <div className="p-10 bg-slate-50 border-t flex flex-col gap-4 shrink-0">
           {completedSteps.size >= steps.length && !activeSubStep ? (
             ...
             >
                {isProcessing ? <Loader2 className="animate-spin" size={20}/> : <>INITIALIZE_TERMINAL <ArrowRight size={20}/></>}
             </button>
           ) : (
             <div className="flex gap-3">
                ...
                >
                  {isProcessing ? <Loader2 size={14} className="animate-spin"/> : <><span className="text-[9px]">Skip Protocol</span> <FastForward size={16} className="group-hover:translate-x-1 transition-transform" /></>}
                </button>
             </div>
           )}
        </div>

...
import { 
  ...
  RefreshCw, ShieldAlert, ArrowLeft, ExternalLink, Info
} from 'lucide-react';
...
const SubscriberOnlinePayment: React.FC<Props> = ({ user, state, onSuccess }) => {
  ...
  const handleInstantPay = async () => {
    ...
    try {
      ...
      setHandshakeStep('Auditing Gateway Configuration...');
      await new Promise(r => setTimeout(r, 800));

      // CRITICAL: Check if admin has actually configured the keys
      const config = selectedGateway.config;
      let isMisconfigured = false;

      if (selectedGateway.id === 'stripe' && (!config.publishableKey || !config.secretKey)) isMisconfigured = true;
      if (selectedGateway.id === 'paypal' && (!config.clientId || !config.secret)) isMisconfigured = true;
      if (selectedGateway.id === 'payfast' && (!config.merchantId || !config.merchantKey)) isMisconfigured = true;
      if (selectedGateway.id === 'jazzcash' && (!config.merchantId || !config.password)) isMisconfigured = true;
      if (selectedGateway.id === 'easypaisa' && (!config.storeId || !config.hashKey)) isMisconfigured = true;

      if (isMisconfigured) {
        throw new Error("GATEWAY_CONFIG_FAULT: Registry node has not provisioned valid API credentials for this handshake.");
      }

      // Phase 2: Remote Node Handshake (Simulation)
      setHandshakeStep(`Redirecting to ${selectedGateway.name} Secure Layer...`);
      await new Promise(r => setTimeout(r, 1500));
      
      // Phase 3: Authorization Flow
      setHandshakeStep('Authorizing Fiscal Transfer...');
      await new Promise(r => setTimeout(r, 2000));

      // Simulated Payment Success Logic
      if (Math.random() < 0.1) { // 10% chance of random user/bank decline
         throw new Error("TRANSACTION_DECLINED: Payment source rejected the handshake. Please verify funds.");
      }

      // Phase 4: Final Ledger Synchronization
      setHandshakeStep('Syncing Ledger Node...');
      await db.processTopup('GATEWAY', user.id, 'user', total);
      // Fix: Removed third argument from activatePackage as it only accepts two (userId, pkgId).
      await db.activatePackage(user.id, selectedPkgId);
      
      // Verification Notification
      db.logNotification(user.id, 'success', 'Package Provisioned', `Link established via ${selectedGateway.name}. Fiscal handshake verified.`);
      ...
      setPaymentError({ 
        msg: err.message || "Handshake Failure: Unknown registry error.",

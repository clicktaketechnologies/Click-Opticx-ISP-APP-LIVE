  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-24">
         ...
         <div className="relative z-10 space-y-12">
            <div className="flex justify-between items-start">
               <div className="space-y-2">
                  <p className="text-[11px] font-black text-white/50 uppercase tracking-[0.4em]">Available Node Liquidity</p>
               ...
               <div className={`px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest border border-white/20 backdrop-blur-md`}>
                  {activeEL ? 'Rescue Active' : isBalanceLow ? 'Refill Needed' : 'Wallet Verified'}
               </div>
            </div>
            ...
            {activeEL && (
              <div className="p-6 bg-black/20 rounded-3xl border border-white/10 space-y-3 animate-in slide-in-from-bottom-2">
                 <div className="flex justify-between items-center">
                    <p className="text-[10px] font-black uppercase tracking-widest opacity-60">Rescue Advance Amount</p>
                    <span className="text-sm font-black italic">Rs. {activeEL.amount.toLocaleString()}</span>
                 </div>
                 <div className="flex justify-between items-center pt-3 border-t border-white/10">
                    <p className="text-[10px] font-black uppercase tracking-widest opacity-60">Due Protocol</p>
            ...
            <div className="flex gap-4">
               {activeEL ? (
                 ...
                 >
                    <ShieldCheck size={18} strokeWidth={4} /> Settle Rescue Debt
                 </button>
               ) : (
                 <>
                   <button 
                    ...
                    className="flex-1 py-5 bg-white text-slate-900 rounded-2xl font-black text-[11px] uppercase tracking-[0.2em] shadow-xl active:scale-95 transition-all flex items-center justify-center gap-3"
                   >
                      <Plus size={18} strokeWidth={4} /> Add Fiscal Credit
                   </button>
                   {!activeEL && (
                     <button 
                       ...
                       title="Authorize Emergency Load"
                     >
        ...
        <div className="bg-red-50 border-2 border-red-200 p-8 rounded-[3rem] flex items-start gap-6 animate-pulse">
           ...
           <div>
              ...
              <p className="text-[10px] text-red-700 font-bold leading-relaxed uppercase">
                 The 72-hour rescue link has expired. Settle the Rs. {activeEL.amount} balance immediately to prevent automated authority suspension.
              </p>
           </div>
        </div>
      )}
      ...
      <div className="space-y-5">
         <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] px-4">Registry Audit Log</h4>
            ...
            {pendingTopups.map(req => (
              <div key={req.id} className="bg-white p-6 rounded-[2.5rem] border border-orange-100 flex items-center justify-between group shadow-sm">
                 <div className="flex items-center gap-5">
                    ...
                    <div>
                       <p className="text-sm font-black text-slate-900 uppercase tracking-tight">Topup Handshake: Rs. {req.amount}</p>
                       <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest">{req.paymentMethod} • Awaiting Sync</p>
                    </div>
                 </div>
              ...
              <div key={l.id} className="bg-white p-6 rounded-[2.5rem] border border-slate-100 flex items-center justify-between group shadow-sm hover:shadow-md transition-all">
                 <div className="flex items-center gap-5">
                    ...
                    <div>
                       ...
                       <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest">{new Date(l.timestamp).toLocaleDateString()} • Audit: {l.method || 'System'}</p>
                    </div>
                 </div>

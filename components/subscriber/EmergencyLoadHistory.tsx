...
const EmergencyLoadHistory: React.FC<Props> = ({ user, state, onBack }) => {
  ...
  const getStatusColor = (status: string) => {
    switch(status) {
      case 'Settled': case 'Paid': return 'text-emerald-500 bg-emerald-50 border-emerald-100';
    ...
    <div className="space-y-8 animate-in slide-in-from-right duration-500 pb-24">
       ...
       <div className="bg-white rounded-[3rem] border border-slate-100 shadow-sm overflow-hidden flex flex-col min-h-[500px]">
          ...
          <div className="divide-y divide-slate-50 overflow-y-auto max-h-[700px] custom-scrollbar">
             {history.length === 0 ? (
               ...
               history.map(log => (
                 <div key={log.id} className="p-8 hover:bg-slate-50 transition-all group border-l-4 border-transparent hover:border-indigo-600">
                    <div className="flex justify-between items-start mb-4">
                       <div className="flex items-center gap-4">
                          <div className={`w-12 h-12 rounded-2xl flex items-center justify-center border shadow-inner transition-all ${log.repaid ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-orange-50 text-orange-600 border-orange-100'}`}>
                    ...
                    {log.extensions && log.extensions.length > 0 && (
                      <div className="mt-4 p-4 bg-indigo-50/50 border border-indigo-100 rounded-2xl space-y-2">
                         ...
                         {log.extensions.map(ext => (
                           <div key={ext.id} className="text-[9px] text-indigo-800 font-bold uppercase leading-none italic pl-2 border-l border-indigo-200">
       ...
       <div className="bg-slate-900 rounded-[3rem] p-10 text-white flex items-center justify-between relative overflow-hidden shadow-2xl">
          <div className="relative z-10">
             <h4 className="text-xl font-black italic uppercase tracking-tighter text-indigo-400">Heuristic Summary</h4>
          ...
          <div className="relative z-10 px-5 py-2 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 rounded-full text-[10px] font-black uppercase">
             Trust Tier: Standard
          </div>
       </div>
    </div>

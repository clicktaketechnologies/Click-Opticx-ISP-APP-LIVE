      ...
      <div className="grid grid-cols-3 gap-3 w-full max-w-sm">
        {[33, 99, 1000].map(v => (
          <button 
            key={v}
            onClick={() => setTarget(v)}
            className={`py-4 rounded-2xl border-2 font-black text-[10px] uppercase tracking-widest transition-all ${target === v ? 'bg-indigo-600 border-indigo-600 text-white shadow-lg' : 'bg-white border-slate-100 text-slate-400 hover:border-indigo-100'}`}
          >
            Tier {v}
          </button>
        ))}

...
const SubscriberActivationFlow: React.FC<Props> = ({ user, state, packageId, isRepayment, onClose, onSuccess }) => {
  ...
  return (
    <div className="fixed inset-0 bg-slate-950/95 backdrop-blur-xl z-[1000] flex items-center justify-center p-6">
      <div className="bg-white rounded-[3.5rem] w-full max-lg shadow-2xl overflow-hidden border-[8px] border-slate-50 animate-in zoom-in duration-300">
        
        {pendingReq && step === 'method' && !isRepayment ? (
          <>
            <div className="p-10 text-center space-y-8 animate-in fade-in duration-300">
          ...
          <>
            {step === 'method' && (
              <div className="p-10 space-y-8 flex flex-col max-h-[85vh]">
                <div className="flex justify-between items-start shrink-0">
                   <div className="space-y-1">
                      <h3 className="text-2xl font-black uppercase italic tracking-tighter text-slate-800">{isRepayment ? 'Repayment Protocol' : 'Activate Package'}</h3>
                      <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Select Fiscal Authorization Method</p>
                   </div>
                   <button onClick={onClose} className="p-2 text-slate-300 hover:text-rose-50 transition-colors"><X size={24}/></button>
                </div>

                <div className="grid grid-cols-1 gap-3 overflow-y-auto no-scrollbar pr-1">
            ...
            {step === 'confirm' && (
              <div className="p-10 space-y-8 animate-in slide-in-from-right duration-300">
                ...
                <div className="p-8 bg-slate-950 rounded-[2.5rem] border border-white/5 space-y-4 shadow-2xl">
                   ...
                   <div className="flex justify-between items-center">
                      <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Handshake Total</span>

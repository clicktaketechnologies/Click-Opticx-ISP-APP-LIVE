...
import { 
  ...
  Cpu, Megaphone, Mic, PhoneCall, Moon, Box, Heart, ChevronDown, ChevronUp, Scale
} from 'lucide-react';
...
const iconMap: Record<string, LucideIcon> = {
  Home, Wallet, Signal, User, Headphones, Zap, LayoutGrid, Monitor, Smartphone, Key, Gauge, Bell, MessageSquare, Globe, ShieldCheck, Clock, Book, History, HelpCircle, UserCheck, Shield, Activity, Wifi, HardDrive, BookOpen, FileText, CreditCard, Target, Compass, Fingerprint, Mic, Sun, Gift, Megaphone, Moon, Box, Heart, Scale
};
...
const SubscriberHome: React.FC<Props> = ({ 
  ...
  const renderSection = (section: AppSection) => {
    ...
    if (section.id === 'status') {
      return (
        <div key="status" className="space-y-6">
            ...
            <div className="mx-4 p-6 bg-amber-50 border-2 border-amber-200 rounded-[2.5rem] flex items-center justify-between shadow-xl animate-in slide-in-from-top-4 duration-500">
               <div className="flex items-center gap-5">
                  ...
                  <div>
                     ...
                     <h4 className="text-sm font-black text-amber-950 uppercase italic tracking-tight">{(pendingPkgReq as any).packageName || 'Activation'} Sync</h4>
                     <p className="text-[9px] text-amber-700 font-bold uppercase mt-1 italic">Pending Administrative Clearance</p>
                  </div>
               </div>
               <button 
                onClick={async () => {
                   if (confirm("ABORT PROTOCOL: Cancel your pending request and restore the terminal state?")) {
                      await db.cancelUniversalRequest(pendingPkgReq.id);
                   }
                }}
                className="p-3 bg-white border border-amber-200 text-amber-400 hover:text-rose-600 rounded-xl transition-all active:scale-90 shadow-sm"
               >
                  <X size={20} />
               </button>
            </div>
          ...
          {/* Heuristic AI Action Grid */}
          <div className="space-y-4 px-4">
            <div className="flex justify-between items-end px-1">
              <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">Intelligence Hub</h3>
              <span className="text-[8px] font-black text-indigo-600 animate-pulse uppercase">Relay Pulse: Active</span>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
               {appearance.showAICalling && (
                  <button 
                    onClick={() => onAction('ai-voice-call')} 
                    className="p-6 bg-slate-900 rounded-[2.5rem] border border-white/5 shadow-2xl text-left group overflow-hidden relative active:scale-95 transition-all md:col-span-1 border-b-8 border-indigo-900 hover:scale-[1.02] hover:-rotate-1"
                  >
                      <div className="relative z-10">
                        ...
                        <h4 className="text-xl font-black text-white uppercase italic tracking-tighter leading-none mb-1">AI Voice</h4>
                        <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest">Real-time Handshake</p>
                      </div>
               ...
               <div className={`grid grid-cols-2 gap-3 ${appearance.showAICalling ? 'md:col-span-2' : 'md:col-span-3'}`}>
                  <button onClick={() => onAction('ai-home')} className="p-5 bg-slate-900 rounded-[2rem] border border-white/5 shadow-xl text-left group overflow-hidden relative active:scale-95 transition-all border-b-8 border-slate-950 hover:scale-[1.02]">
                     ...
                     <h4 className="text-[10px] font-black text-white uppercase italic tracking-tighter">AI Dashboard</h4>
                     <Activity size={40} className="absolute -right-2 -bottom-2 opacity-10 text-indigo-400 pointer-events-none" />
                  </button>
                  {appearance.showAIChat && (
                    <button onClick={() => onAction('aichat')} className="p-5 bg-indigo-600 rounded-[2rem] border border-indigo-400 shadow-xl text-left group overflow-hidden relative active:scale-95 transition-all border-b-8 border-indigo-800 hover:scale-[1.02]">
                      ...
                      <h4 className="text-[10px] font-black text-white uppercase italic tracking-tighter">AI Chat Terminal</h4>
          ...
          {/* Core Plan Visualizer */}
          <div className={`mx-4 rounded-[3rem] p-8 text-white relative overflow-hidden shadow-2xl transition-all duration-700 border-b-8 hover:scale-[1.02] ${user.status === UserStatus.ACTIVE && !isExpired ? 'bg-indigo-600 border-indigo-800 shadow-indigo-200' : 'bg-rose-600 border-rose-800 shadow-rose-200'}`}>
            <div className="relative z-10 space-y-8">
              <div className="flex justify-between items-start">
                 <div className="space-y-1">
                    <div className="flex items-center gap-2">
                       <div className={`w-2.5 h-2.5 rounded-full ${!isExpired ? 'bg-emerald-400 animate-pulse' : 'bg-white'}`}></div>
                       <span className="text-[10px] font-black uppercase tracking-[0.2em] opacity-80">{isExpired ? 'Link Expired' : 'Link Established'}</span>
                    </div>
                    <h2 className="text-3xl font-black tracking-tighter uppercase italic truncate max-w-[240px]">{currentPkg?.name || 'NODE_DORMANT'}</h2>
                 </div>
                 <div className="w-12 h-12 bg-white/10 rounded-2xl flex items-center justify-center border border-white/20 backdrop-blur-md overflow-hidden shrink-0 shadow-lg">
              ...
              {!pendingPkgReq && (
                <div className="flex gap-3">
                  <button onClick={() => setShowActivation(true)} className="flex-1 py-5 bg-white text-slate-900 rounded-2xl font-black text-[11px] uppercase tracking-widest shadow-xl active:scale-95 transition-all flex items-center justify-center gap-2 border-b-4 border-slate-200">
                    <Zap size={16} fill="currentColor" /> {isExpired ? 'Sync Renewal' : 'Modify Tier'}
                  </button>
    ...
    if (section.id === 'rescue' && activeEL) {
      return (
        <div key="rescue" className="space-y-4 px-4">
          <div className={`p-8 rounded-[3rem] border-4 shadow-xl flex flex-col gap-6 animate-in slide-in-from-bottom-4 duration-500 border-b-8 hover:scale-[1.02] ${isELPastDue ? 'bg-rose-600 border-rose-400 text-white border-b-rose-800' : isELPending ? 'bg-indigo-600 border-indigo-400 text-white border-b-indigo-800' : 'bg-amber-600 border-amber-400 text-white border-b-amber-800'}`}>
            <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
                <div className="flex items-center gap-5 text-center sm:text-left">
                  <div className="w-16 h-16 bg-white/20 rounded-3xl flex items-center justify-center backdrop-blur-md border border-white/10 shrink-0 shadow-lg">
                      {isELPending ? <RefreshCw size={36} className="animate-spin" /> : <ShieldAlert size={36} />}
                  </div>
                  <div className="flex-1">
                      <h4 className="text-xl font-black uppercase tracking-tight leading-none mb-1">
                        {isELPending ? 'Syncing Advance...' : isELPastDue ? 'Overdue Risk' : 'Rescue Link Ready'}
                      </h4>
                      <p className="text-[10px] font-bold uppercase opacity-90 tracking-widest leading-relaxed">
                        {isELPending ? `Verifying Rs. ${activeEL.amount} advance logic. Lock: ${timeLeft}` : isELPastDue ? `Registry debt overdue. Settle to restore node health.` : `Rs. ${activeEL.amount} provisioned. Settlement due ${new Date(activeEL.expiryTimestamp).toLocaleDateString()}.`}
                      </p>
                  </div>
                </div>
                {!isELPending && (
                  <button onClick={() => onAction('wallet')} className="w-full sm:w-auto px-8 py-5 bg-white text-slate-950 rounded-2xl font-black text-xs uppercase tracking-widest shadow-2xl active:scale-95 flex items-center justify-center gap-2 border-b-4 border-slate-200">
                    Settle Ledger <ArrowRight size={18} />
                  </button>
                )}
            </div>
          </div>
    ...
    if (section.id === 'credit') {
      return (
        <button 
          ...
          className="bg-white rounded-[2.5rem] p-6 border-b-8 border-slate-100 shadow-xl flex items-center justify-between group active:scale-[0.98] transition-all mx-4 hover:shadow-2xl hover:-translate-y-1 hover:border-indigo-200"
        >
           <div className="flex items-center gap-5">
              ...
              <div className="text-left space-y-1">
                 <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Trust Integrity Rank</p>
                 <div className="flex items-center gap-2">
                    <span className="text-2xl font-black italic tracking-tighter text-slate-900">{user.creditScore}</span>
                    <span className={`text-[9px] font-black uppercase ${scoreRange.color}`}>{scoreRange.label} Tier</span>
                 </div>
              </div>
           </div>
    ...
    if (section.id === 'fiscal-summary' && appearance.showWallet) {
      return (
        <div key="fiscal-summary" className="bg-white rounded-[2.5rem] border-b-8 border-slate-100 shadow-xl overflow-hidden flex flex-col mx-4 animate-in fade-in duration-700 hover:shadow-2xl transition-all">
          <div className="p-6 bg-slate-50/50 border-b border-slate-50 flex items-center justify-between">
              ...
              <span className="text-[9px] font-black text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-md border border-emerald-100 uppercase italic">Ledger Synced</span>
          </div>
          <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-4">
                 <div className="flex justify-between items-end"><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Package Expiry</p><p className={`text-xs font-black uppercase tracking-tight ${isExpired ? 'text-red-600' : 'text-slate-900'}`}>{user.expiryDate ? new Date(user.expiryDate).toLocaleDateString() : 'NO_SYNC'}</p></div>
              ...
              <div className="grid grid-cols-2 gap-4">
                 <button onClick={() => onAction('billing')} className="p-4 bg-slate-50 rounded-2xl border border-slate-100 shadow-sm text-left"><p className="text-[8px] font-black text-slate-400 uppercase mb-1">Unpaid Bills</p><p className="text-lg font-black text-slate-900">{activeUnpaidCount}</p></button>
                 <button onClick={() => onAction('wallet')} className="p-4 bg-slate-50 rounded-2xl border border-slate-100 shadow-sm text-left"><p className="text-[8px] font-black text-slate-400 uppercase mb-1">Wallet Node</p><p className={`text-lg font-black ${user.balance > 0 ? 'text-red-600' : 'text-emerald-600'}`}>Rs. {user.balance.toLocaleString()}</p></button>
              </div>
          </div>
        </div>
      );
    ...
    const gridClasses: Record<number, string> = { 1: 'grid-cols-1', 2: 'grid-cols-2', 3: 'grid-cols-2 md:grid-cols-3', 4: 'grid-cols-2 md:grid-cols-4' };
      ...
      <div key={section.id} className="space-y-6 px-4">
        <div className="flex justify-between items-end px-2">
          ...
          {isDirectory && allItems.length > 4 && (
            ...
            >
              {isDirectoryExpanded ? <><ChevronUp size={14}/> Show Less</> : <><ChevronDown size={14}/> View {allItems.length} Services</>}
            </button>
          )}
        </div>
          ...
          {items.map(item => {
            ...
            return (
              <button 
                ...
                style={{ borderBottom: `8px solid ${item.swatch ? item.swatch + '55' : '#f1f5f9'}` }}
                className="p-6 bg-white rounded-[2.5rem] border border-slate-100 shadow-[0_20px_50px_-12px_rgba(0,0,0,0.08)] hover:shadow-[0_25px_60px_-12px_rgba(0,0,0,0.12)] hover:-translate-y-1 transition-all duration-300 active:scale-95 group text-left flex flex-col gap-4 relative overflow-hidden h-full"
              >
                <div className="absolute inset-0 opacity-5 transition-opacity group-hover:opacity-10 pointer-events-none" style={{ backgroundColor: item.swatch || '#f1f5f9' }} />
                <div 
                  className="w-14 h-14 rounded-2xl flex items-center justify-center border shadow-sm transition-all group-hover:scale-110 group-hover:rotate-6 z-10"
                  style={{ background: `linear-gradient(135deg, ${item.swatch ? item.swatch + '20' : '#f8fafc'} 0%, ${item.swatch ? item.swatch + '40' : '#f1f5f9'} 100%)`, color: item.swatch || '#94a3b8', borderColor: item.swatch ? item.swatch + '30' : '#f1f5f9' }}
                >
                ...
                <div className="z-10">
                  ...
                  <p className="text-[8px] text-slate-400 font-bold uppercase tracking-widest leading-tight">{item.category} Registry</p>
                </div>
                <Icon size={100} className="absolute -right-6 -bottom-6 opacity-[0.03] text-slate-950 pointer-events-none group-hover:scale-150 transition-transform duration-700" />
              </button>
            );
          })}
        </div>

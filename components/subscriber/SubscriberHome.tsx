...
const iconMap: Record<string, LucideIcon> = {
  Home, Wallet, Signal, User, Headphones, Zap, LayoutGrid, Monitor, Smartphone, Key, Gauge, Bell, MessageSquare, Globe, ShieldCheck, Clock, Book, History, HelpCircle, UserCheck, Shield, Activity, Wifi, HardDrive, BookOpen, FileText, CreditCard, Target, Compass, Fingerprint, Mic, Sun, Gift, Megaphone, Moon, Box, Heart, BarChart3, Cloud, RefreshCw
};
...
const SubscriberHome: React.FC<Props> = ({
  ...
  const renderSection = (section: AppSection) => {
    ...
    if (section.id === 'credit') {
      return (
        <button
          ...
          className="bg-white rounded-[2.5rem] p-6 border-b-8 border-slate-100 shadow-xl flex items-center justify-between group active:scale-[0.98] transition-all mx-4 hover:shadow-2xl hover:-translate-y-2 hover:border-indigo-200"
        >
          <div className="flex items-center gap-5">
    ...
    if (section.id === 'fiscal-summary' && appearance.showWallet) {
      return (
        <div key="fiscal-summary" className="bg-white rounded-[2.5rem] border-b-8 border-slate-100 shadow-xl overflow-hidden flex flex-col mx-4 animate-in fade-in duration-700 hover:shadow-2xl transition-all hover:-translate-y-1">

               ...
               <div className="flex flex-col md:flex-row gap-6 items-center justify-between w-full">
                  <div>
                     ...
                     <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest mt-1">
                        Amount Due: <span className="text-rose-600">{curr} {( || 0).toLocaleString()}</span>
                     </p>
                  </div>
                     ...
                     <div ref={invoiceRef} className="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-sm p-10 space-y-10">
                           ...
                           <div className="space-y-3">
                              {invoice.items.map(item => (
                                 <div key={item.id} className="p-5 bg-white border border-slate-100 rounded-[2rem] flex items-center justify-between group hover:border-blue-200 hover:shadow-xl hover:shadow-blue-500/5 transition-all">
                                    ...
                                    <div className="text-right">
                                       <p className="text-xl font-black italic text-slate-900 tracking-tighter tabular-nums">{curr} {( || 0).toLocaleString()}</p>
                        ...
                        <div className="p-10 bg-gradient-to-br from-slate-950 to-indigo-950 rounded-[2.5rem] flex justify-between items-center shadow-2xl relative overflow-hidden">
                           ...
                           <div className="text-right relative z-10">
                              <p className="text-4xl md:text-5xl font-black italic tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-indigo-300 drop-shadow-lg">{curr} {( || 0).toLocaleString()}</p>
                           </div>
                        </div>
                     </div>
                  </div>
                        ...
                        relatedHistory.map(l => (
                           <div key={l.id} className="p-6 bg-white border border-slate-100 rounded-[2rem] flex items-center justify-between group hover:shadow-lg hover:border-blue-100 transition-all">
                              ...
                              <p className={`text-xl font-black italic tracking-tighter ${l.type === LedgerType.DEBIT ? 'text-red-600' : 'text-green-600'}`}>
                                 {l.type === LedgerType.DEBIT ? '-' : '+'} {curr} {( || 0).toLocaleString()}
                              </p>
                           </div>
                        ))
                     )}
